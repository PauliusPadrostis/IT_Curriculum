...
Slide separation: one section per slide. Never use explicit PageBreak
              paragraphs between slides.
```
...
Do NOT fake vertical centering with an approximate `spacing.before`
value. That creates visibly inconsistent title slides across lessons.
Instead, build slide 1 as a single borderless table that fills the
usable content area, shade the cell navy, set the cell vertical
alignment to CENTER, and place the module/title text block inside that
cell. This makes the title position deterministic across short and long
lesson titles.
...
If an extracted objective starts with a lowercase Lithuanian letter but
is clearly a full sentence or goal, capitalize the first letter for the
slide output.
...
- **Validation:** Before delivery, verify the rendered PDF itself: 6 pages,
  no `?` inside Lithuanian words, title slide visually centered, and no
