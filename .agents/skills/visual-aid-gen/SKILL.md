...
If an extracted objective starts with a lowercase letter but is clearly a
full sentence or goal, capitalize the first letter in the visual aid
unless the line intentionally begins with an acronym, symbol, or code token.
...
Capitalize sentence starts consistently on generated term labels and
definitions. Do not ship accidental lowercase openings caused by raw
extraction or partial summarization.
...
### 3e-ii. Mechanical Em Dash Strip (mandatory, non-skippable)
...
6. **Replacement-character hard gate:** Search the sidecar for `?` inside
   Lithuanian words or key headings. Zero tolerance for patterns such as
   `Interneti?`, `PRANE?TI`, `u?duotis`, `ta?`, or any `\p{L}\?\p{L}`
   match. Treat this as rendering or encoding corruption, not a typo.
7. **Sentence-start capitalization check:** On slides 3-5, every sentence
   or bullet sentence must start with a capital letter unless it begins
   with an acronym, number, symbol, or teacher-approved literal token.
8. **Fix and regenerate** if any issues are found. Do not present a
   document with known errors.

---

## Step 5: PDF Conversion and Render Verification
...
### 5b. Verify the rendered PDF before delivery

After conversion, verify the final PDF itself, not just the source .docx.
At minimum:

- confirm the PDF has exactly 6 pages;
- extract text from the PDF and compare it against the sidecar for gross
  drift or replacement-character corruption;
- block delivery if any Lithuanian word in the PDF contains `?`;
- raster-preview page 1 and at least one content slide to confirm the
  title slide is visually centered and the content is not overflowed or
  clipped.

If the PDF fails any of these checks, fix the source and regenerate. Do
not hand off a PDF that only "looks right in the docx."
