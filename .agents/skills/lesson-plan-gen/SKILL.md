...
- **If it does not exist:** warn the teacher:
  ...
  > Šablonas: `.Codex/skills/module-qa/references/module_design_template.md`
...
**Teacher_Plan hard gate (mandatory, do not skip):**
After the generic QA pass, run a second targeted scan on the final Teacher_Plan text.
This is a hard gate, not a "best effort" proofreading step. Do not present the file
until the scan is clean.

Inspect these sections explicitly because recent corruption clustered there:
- metadata table (`Tipas`, `Klase`, `Trukme`, `Forma`, `Temos ribos`)
- `Pamokos tikslai`
- `Pamokos aprasymas (dienynui)`

Scan for:
- any direct matches from `_references/lt-mistakes.yaml`
- dropped-diacritic Teacher_Plan forms in metadata/scope prose, especially broken accusatives/genitives like
  `atstuma`, `ritma`, `teoriju`, `akiu`, `ranku`, `riesu`
- recurring corrupted stems and morphology families seen in Teacher_Plan batches:
  `si pamoka`, `siame`, `mazdaug`-family corruption, `sekundziu`-family corruption,
  `slaptazodziu`-family corruption, `konkrec*` stem corruption, `suk*` forms where the
  intended stem is `sukc-`, `siuntejas`-family corruption, `veiksma`-family corruption,
  `ketinim*`-family corruption, `tycia`-family corruption, `saltini`-family corruption,
  `pazeistos`, `skiriamosios`-family corruption, `irenginiu`-family corruption,
  `didziausi*`, `siunciam*`, `nelegalius`, `savartynus`, `imsis`, `imsites`

If any hit remains:
1. patch the `.docx`,
2. regenerate the sidecar from the patched file,
3. rerun the hard gate,
4. only then present the file.
