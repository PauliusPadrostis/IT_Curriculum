...
**Decision**: Answer keys will be generated by a dedicated `answer-key-gen` skill, not embedded in student-task-gen or assessment-task-gen. Answer keys are student-facing PDFs serving both A and P lessons. They include correct answers with explanations, alternative valid answers, common wrong answers with explanations, and multiple code versions for programming tasks.

**Context**: Answer keys serve two lesson types (A and P) generated by different skills. Embedding in either skill would create a dependency tangle. Students receive answer keys after grading (A) or during self-check (P).

**Rationale**: A dedicated skill can read output from both assessment-task-gen and student-task-gen to produce consistent answer keys. Backward design order: assessment-task-gen → student-task-gen (P extension) → answer-key-gen.
