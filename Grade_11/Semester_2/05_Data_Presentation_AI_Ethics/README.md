# Data – Presentation, AI & Ethics

**Grade:** Grade 11  
**Semester:** Semester 2

---

> **Teacher note:** AI ethics section needs current examples — update annually.

## Lesson Index

| # | Folder | Type |
|---|--------|------|
| 101 | `101_L - Choosing representation by question type/` | 📖 Lesson |
| 102 | `102_L - Chart misuse and misleading visuals/` | 📖 Lesson |
| 103 | `103_L - Map use cases (when geography matters, when it/` | 📖 Lesson |
| 104 | `104_F - Formative check/` | 📌 Formative check |
| 105 | `105_I - Dataset → choose appropriate visualization/` | 🔗 Integration practice |
| 106 | `106_I - Write a short conclusion data → claim/` | 🔗 Integration practice |
| 107 | `107_I - Add AIethics lens identify what could go wrong/` | 🔗 Integration practice |
| 108 | `108_L - Neural networkAI application interpretation/` | 📖 Lesson |
| 109 | `109_L - What data drives AI outcomes (training data impact)/` | 📖 Lesson |
| 110 | `110_L - Ethical risks (bias, privacy) + regulation/` | 📖 Lesson |
| 111 | `111_F - Formative check/` | 📌 Formative check |
| 112 | `112_P - Short argument responses (ethicsregulation)/` | 🏃 Practice exam |
| 113 | `113_A - Analytical extension (VBE 1 style)/` | 📝 Assessment |
| 114 | `114_L - Collaboration models for hybrid work/` | 📖 Lesson |
| 115 | `115_L - Roles, responsibilities, and workflow contracts/` | 📖 Lesson |
| 116 | `116_L - Tool discipline (not tool shopping)/` | 📖 Lesson |
| 117 | `117_L - Feedback and conflict handling in digital teams/` | 📖 Lesson |
| 118 | `118_F - Formative check/` | 📌 Formative check |
| 119 | `119_I - Coordination simulation/` | 🔗 Integration practice |
| 120 | `120_A - Practical coordination check/` | 📝 Assessment |
| 121 | `121_L - TCPIP model as a reasoning tool (not memorization)/` | 📖 Lesson |
| 122 | `122_L - IP addressing essentials/` | 📖 Lesson |
| 123 | `123_L - Protocols + ports (functional mapping)/` | 📖 Lesson |
| 124 | `124_L - Domain, URL anatomy, DNS resolution path/` | 📖 Lesson |
| 125 | `125_F - Formative check/` | 📌 Formative check |
| 126 | `126_I - Scenario-based diagnostics mini-lab/` | 🔗 Integration practice |
| 127 | `127_A - Structured test (VBE 1 style)/` | 📝 Assessment |

---
