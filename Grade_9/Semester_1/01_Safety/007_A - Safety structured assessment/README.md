# 007_A — Saugos struktūrinis vertinimas

| Laukas          | Reikšmė                                          |
|-----------------|---------------------------------------------------|
| **Tipas**       | 📊 A — Vertinimas                                 |
| **Klasė**       | 9                                                 |
| **Semestras**   | 1                                                 |
| **Tema**        | Sauga                                             |
| **Eilės nr.**   | 007 iš metų sekos                                 |
| **Būsena**      | 🚧 WIP                                             |

---

## Apžvalga

Formalus struktūrinis vertinimas, apimantis visą saugos modulį: ergonomiką, privatumą ir paskyrų saugą, internetines rizikas ir skaitmeninių technologijų poveikį aplinkai. Testas vykdomas Testmoz platformoje ir apima uždarus klausimus, trumpus atvirus atsakymus bei scenarijų analizę. Rezultatas fiksuojamas kaip pažymys.

**Programos nuoroda:** Saugus elgesys F1.1–F1.4, F2.1–F2.4, F3.1–F3.4 — visų saugos sričių vertinimas.

---

## Mokymosi tikslai

Pasibaigus pamokai, mokinys gebės:

1. Pademonstruoti saugos modulio žinių įsisavinimą atsakant į struktūrinius klausimus.
2. Pritaikyti išmoktas taisykles naujose situacijose analizuojant scenarijus.

---

## Sėkmės kriterijai

Mokinys pasiekė tikslą, kai:

- Gali teisingai atsakyti į uždarus klausimus iš visų 4 temų.
- Gali trumpai ir tiksliai paaiškinti sąvokas (pvz., 2FA, phishing, e-atliekos).
- Gali scenarijaus analizėje identifikuoti problemą ir pasiūlyti pagrįstą veiksmą.

---

## Bloom taksonomijos lygis

**Lygis 3 — Taikyti**

Vertinime mokiniai ne tik atkuria žinias, bet turi pritaikyti taisykles naujose situacijose (scenarijų analizė). Uždari klausimai tikrina prisimintinimą ir supratimą, tačiau scenarijų dalis kelia kognityvinę paklausą iki taikymo lygio.

---

## Vertinimo būdas

| Laukas               | Reikšmė                                          |
|----------------------|---------------------------------------------------|
| **Vertinimo formatas** | Testas (Testmoz platforma)                      |
| **Vertinimo skalė**   | 1–10                                             |
| **Klausimų tipai**    | Uždari (pasirinkimas), trumpi atviri, scenarijų analizė |
| **Trukmė**            | ~35 min.                                         |
| **Sąlygos**           | Individualiai, be pagalbinių priemonių            |

---

## Dažniausios klaidos

Bus papildyta po pirmos pamokos vykdymo.

---

## Pamokos eiga (santrauka)

| Laikas    | Etapas                     | Veikla                                  |
|-----------|----------------------------|-----------------------------------------|
| 0–3 min   | Trumpas paaiškinimas       | Vertinimo formato instrukcija            |
| 3–38 min  | Vertinimo užduotis         | Mokiniai dirba individualiai Testmoz     |

---

## Trukmė

**1 pamoka (40 min)**

---

## Namų darbas

Nėra.

---

## 📁 Reikalingi failai

| Failas                | Būsena | Patikrinta | Pastaba                                           |
|-----------------------|--------|------------|---------------------------------------------------|
| Teacher_Plan.docx     | ✅     | ❌         | Privalomas                                        |
| Assessment_Task.xlsx  | ✅     | ❌         | **Privalomas** — Testmoz importo failas           |
| Rubric.pdf            | ✅     | ❌         | **Bent vienas privalomas** — vertinimo kriterijai |
| Rubric.docx           | ✅     | ❌         | Redaguojama rubrika (bonus)                       |
| Answer_Key.docx       | ❌     | ❌         | Atsakymų raktas (mokytojo vidinis dokumentas)      |

---

## Papildomi ištekliai

Nėra.
